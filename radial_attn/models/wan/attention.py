...
class WanSparseAttnProcessor:
    ...
    def __call__(
        ...
        if attn.cross_attention_dim_head is not None: # case for cross attention
            ...
            if self.use_sp:
            # Convert to [b, h, s, d] for distributed processing
            ...
            # Now both use_sp=True and use_sp=False cases have [b, s, h, d]
            ...
            if numeral_timestep < self.dense_timestep or self.layer_idx < self.dense_block or self.sparse_type == "dense":
                hidden_states = RadialAttention(
                    ...
                    mask_map=self.mask_map, sparsity_type="dense", 
                    block_size=128, decay_factor=self.decay_factor, 
                    model_type="wan", pre_defined_mask=None, 
                    use_sage_attention=self.use_sage_attention
                )
            else:
                hidden_states = RadialAttention(
                    query=query, key=key, value=value, 
                    mask_map=self.mask_map, sparsity_type="radial", 
                    block_size=128, decay_factor=self.decay_factor, 
                    model_type="wan", pre_defined_mask=None, 
                    use_sage_attention=self.use_sage_attention
                )
        
        # RadialAttention returns [b, s, h*d] - already 3D and flattened
