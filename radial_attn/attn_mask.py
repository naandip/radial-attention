...
def gen_log_mask_shrinked(query, s, video_token_num, num_frame, block_size=128, sparse_type="log", decay_factor=0.5, model_type=None):
    ...
    for i in range(num_frame):
        for j in range(num_frame):
            ...
            # Calculate the actual available space in final_log_mask
            block_row_end = min(block_row_start + block_mask.shape[0], final_log_mask.shape[0])
            block_col_end = min(block_col_start + block_mask.shape[1], final_log_mask.shape[1])

            # Get the actual dimensions we can use
            actual_rows = block_row_end - block_row_start
            actual_cols = block_col_end - block_col_start
            ...
            final_log_mask[block_row_start:block_row_end, block_col_start:block_col_end] = torch.logical_or(
                ...
                block_mask[:actual_rows, :actual_cols]
            )
...
def SpargeSageAttnBackend(query, key, value, mask_map=None, video_mask=None, pre_defined_mask=None, block_size=128):
    ...
    # sparse case - need to pad the mask to match full sequence length
    arch = get_cuda_arch_versions()[query.device.index]
    
    # video_mask is [video_blocks, video_blocks]
    # We need to expand it to match the full sequence length
    seq_blocks = (seq_len + block_size - 1) // block_size
    key_blocks = (key.shape[1] + block_size - 1) // block_size
    
    # Create a full mask padded to sequence length
    full_video_mask = torch.zeros((seq_blocks, key_blocks), device=query.device, dtype=torch.bool)
    video_blocks = video_mask.shape[0]
    full_video_mask[:video_blocks, :video_blocks] = video_mask
    
    # For text tokens (beyond video), allow full attention
    if seq_blocks > video_blocks:
        full_video_mask[video_blocks:, :] = True  # text queries attend to all
    if key_blocks > video_blocks:
        full_video_mask[:, video_blocks:] = True  # all queries attend to text keys
    
    converted_mask = sparge_mask_convert(mask=full_video_mask, block_size=block_size, arch=arch)
...
def FlashInferBackend(query, key, value, mask_map=None, pre_defined_mask=None, bsr_wrapper=None, is_padded=False, original_video_tokens=None):
    """
    Args:
        query, key, value: [batch, seq, heads, dim] - if is_padded=True, seq includes padding for video portion
        is_padded: whether the video portion has been padded to block_size multiple
        original_video_tokens: the original video token count before padding
    """
    batch_size = query.shape[0]
    
    # Determine the actual video token boundary
    video_tokens = original_video_tokens if is_padded else mask_map.video_token_num
    
    # FlashInfer needs (seq, heads, dim), reshape only here
    query_shd = rearrange(query[:, :video_tokens], "b s h d -> (b s) h d")
    key_shd = rearrange(key[:, :video_tokens], "b s h d -> (b s) h d")
    value_shd = rearrange(value[:, :video_tokens], "b s h d -> (b s) h d")
    ...
    if pre_defined_mask is not None:
        ...
        if is_padded:
            o_video = o_video[:original_video_tokens]
            o_text = o_text  # text tokens unchanged
...
def RadialAttention(query, key, value, mask_map=None, sparsity_type="radial", block_size=128, decay_factor=1, model_type=None, pre_defined_mask=None, use_sage_attention=False):
    ...
    # if query.ndim <= 3: query = query.unsqueeze(0)
    # if key.ndim   <= 3: key   = key.unsqueeze(0)
    # if value.ndim <= 3: value = value.unsqueeze(0)
    # print(f"{query.shape=}")
    # print(f"{key.shape=}")
    # print(f"{value.shape=}")
    ...
    # print(f"Video mask obtained, {video_mask.shape=}")
    if backend == "flashinfer":
        # Calculate padded video token number
        padded_video_tokens = ((mask_map.video_token_num + block_size - 1) // block_size) * block_size
        actual_video_blocks = mask_map.video_token_num // block_size
        padded_video_blocks = padded_video_tokens // block_size
        
        # Pad the video_mask if needed
        if padded_video_blocks > actual_video_blocks:
            padded_mask = torch.zeros((padded_video_blocks, padded_video_blocks), device=query.device, dtype=torch.bool)
            padded_mask[:actual_video_blocks, :actual_video_blocks] = video_mask
            video_mask = padded_mask
        
        # Create padded query/key/value by inserting padding WITHIN the video portion
        padding_needed = padded_video_tokens - mask_map.video_token_num
        if padding_needed > 0:
            # Insert padding after video tokens but before text tokens
            query_padded = torch.cat([
                query[:, :mask_map.video_token_num],
                torch.zeros(batch_size, padding_needed, num_head, hidden_dim, device=query.device, dtype=query.dtype),
                query[:, mask_map.video_token_num:]  # text tokens after padding
            ], dim=1)
            key_padded = torch.cat([
                key[:, :mask_map.video_token_num],
                torch.zeros(batch_size, padding_needed, num_head, hidden_dim, device=key.device, dtype=key.dtype),
                key[:, mask_map.video_token_num:]
            ], dim=1)
            value_padded = torch.cat([
                value[:, :mask_map.video_token_num],
                torch.zeros(batch_size, padding_needed, num_head, hidden_dim, device=value.device, dtype=value.dtype),
                value[:, mask_map.video_token_num:]
            ], dim=1)
        else:
            query_padded = query
        ...
        bsr_wrapper.plan(
            indptr=indptr,
            indices=indices,
            M=padded_video_tokens,
            N=padded_video_tokens,
            R=block_size,
            C=block_size,
        ...
        # Pass full tensors with padding info
        return FlashInferBackend(
            query_padded, key_padded, value_padded, 
            mask_map, pre_defined_mask, bsr_wrapper,
            is_padded=(padding_needed > 0),
            original_video_tokens=mask_map.video_token_num
        )
